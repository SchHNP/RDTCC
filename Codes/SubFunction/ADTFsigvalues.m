function [gamma2_adtf,mean_adtf] = ADTFsigvalues(ts,low_freq,high_freq,p,fs,shufftimes, siglevel)
%[new_gamma2, gamma2_adtf] = ADTFsigvalues(ts,low_freq,high_freq,p,fs,shufftimes, siglevel)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% nfreq = floor(high_freq-low_freq+1);
...
gamma2_adtf = cell(1,nreps);

% Number of surrogate datasets to generate
h = waitbar(0,'ADTF for surrogate time series, wait...');
sum_adtf = 0;
for i=1:nreps
...
%     end
    ...
    gamma2_set = ADTF1(newts,low_freq,high_freq,p,fs,1);
    gamma2_adtf{i} = gamma2_set;
    
    %########################################
    sum_adtf = sum_adtf+gamma2_set;  %我自己加的
    %########################################
...
%     new_gamma2(sig_size,:,:,:,:) = [];
    ...
    PerStr = fix((i/nreps)*100);
    str = ['ADTF for surrogate time series, wait...',num2str(PerStr),'%'];
    waitbar(i/nreps,h,str);
end

%##################################################
mean_ = sum_adtf/nreps;   %我自己加的
mean_adtf = mean(mean_,4);
%##################################################
close(h);
