...
% Non-normalized DTF
theta2 = size(nchan, nchan, nfre);
for k = 1:nfre

    Af = zeros(nchan,nchan);
    fre = tot_range(k);

    expT = exp(-pi*fre*dt*((1:p+1)-1)*2i);
    expT1 = repmat(reshape(expT, 1, 1, []), size(A, 1), size(A, 2));
    TS_E = A .* expT1;
    TS_E = sum(TS_E, 3);
    Af = Af - TS_E;
%     for i = 1:nchan
%         for j = 1:nchan
% 
% %             Af(i,j) = Af(i,j) - sum(squeeze(squeeze(A(i,j,1:p+1)).*expT));
%             for h = 1:p+1
...
% Normalized DTF
X = sum(theta2, 2);
gamma2 = theta2./repmat(X, 1, nchan, 1);

% gamma2 = size(nchan, nchan, nfre);
% for k = 1:nfre
% 
%     
%     for i = 1:nchan
